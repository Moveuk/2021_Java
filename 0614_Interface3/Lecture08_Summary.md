...
```
 ...
 쉽게 정리하자면 인스턴스 멤버 클래스(static이 아닌 중첩 클래스)는 정적 필드와 정적 메소드를 바깥 클래스에 초기화 시키지 못한다.   
하지만, 정적(static인 중첩클래스) 멤버 클래스는 인스턴스와 정적을 불문하고 모든 필드와 메소드를 초기화 가능하다.
 ...
 **인스턴스 멤버 클래스(B)**안에서는 오히려 바깥 클래스의 **모든 필드와 메소드에 접근**하여 사용 및 정의할 수 있지만, **정적(static) 멤버 클래스(C)** 안에서는 **바깥 클래스의 정적 필드와 메소드 필드에만 접근**할 수 있고, 인스턴스 필드와 메소드 클래스에는 접근할 수 없다.   
...
### 교재 398p : 9.3.3 로컬 클래스에서 사용 제한  
<br/>
 
 우리가 알아둘 것 : 로컬 클래스에서 사용되는 매개 변수와 로컬 변수는 **모두 final 특성을 갖는 다는 것**만 알면 된다.


<br/><br/>   
<hr/>
   
### 교재 400p : 9.3.4 중첩 클래스에서 바깥 클래스 참조 얻기      
<br/> 
